...
\gdef\referenceframett#1{
  \begin{numberedframe}{Reference: \texttt{MPI_#1}}
    \small
    \lstinputlisting{mplreference/MPI_#1.tex}
...
\begin{exercises}
  \makeatletter
  \frame{\frametitle{List of exercises}\def\numberline{\par\noindent}\@starttoc{exe}}
  \makeatother
\end{exercises}
...
\begin{numberedframe}{World communicator}
  ...
  Most MPI functions will be methods
  of a communicator object.
\end{numberedframe}

\begin{numberedframe}{Processor name}
  \input{mplnote-processor-name.cut}
  (One of the few functions not associated with a communicator.)
\end{numberedframe}
...
\begin{exerciseframe}[commrank]
  \begin{enumerate}
  \item Use the rank/size functions to let each process print out\\
    \lstinline[language=shell]{Hello from process 3 out of 5}
  \item Let just one process print out\\
    \lstinline[language=shell]{Hello from 5 processes}
  \end{enumerate}
\end{exerciseframe}
...
\begin{numberedframe}{Reduction operator}
  ...
  \input{mplnote-allreduce-operator.cut}
\end{numberedframe}
\referenceframe{Allreduce}
\referenceframe{Reduce}

\begin{exerciseframe}[reduce]
  \begin{enumerate}
  \item
    Let each process have a random value.
  \item 
    Normalize this number by the sum of the values.
  \item 
    For bonus points sum the normalized values and check that they sum to~one.
  \end{enumerate}
\end{exerciseframe}
...
\end{numberedframe}
\begin{numberedframe}{Reduction and non-root processes}
  \input{mplnote-send-vs-recv-buffer.cut}
\end{numberedframe}
...
\end{numberedframe}
%% \begin{numberedframe}{Sending automatic arrays}
%%   \input{mplnote-sending-automatic-arrays.cut}
%% \end{numberedframe}
...
\begin{numberedframe}{Communicator comparing}
  \input{mplnote-communicator-comparing.cut}
\end{numberedframe}
...
\begin{numberedframe}{Cartesian communicator}
  \input{mplnote-cartesian-communicator.cut}
\end{numberedframe}

\begin{numberedframe}{Dims create}
  \input{mplnote-dims-create.cut}
\end{numberedframe}

\begin{numberedframe}{Cartesian communicator create}
  \input{mplnote-cartesian-communicator-create.cut}
\end{numberedframe}

\begin{numberedframe}{Get the dimensions object}
  \input{mplnote-get-the-dimensions-object.cut}
\end{numberedframe}

\begin{numberedframe}{Rank to coord translation}
  \input{mplnote-rank-to-coord-translation.cut}
\end{numberedframe}

\begin{numberedframe}{Cartesian shifting}
  \input{mplnote-cartesian-shifting.cut}  
\end{numberedframe}
...
\begin{numberedframe}{Info objects}
  \input{mplnote-info-objects.cut}
\end{numberedframe}
