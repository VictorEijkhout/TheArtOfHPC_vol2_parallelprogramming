...
\begin{instructor}
  Finding the max is a reduction with a max operator.\\
  You could gather and then traverse the array of values,
  but that's a time and space bottleneck:\\
  the `root' needs as much space as the other processes combined,
  and the root is active while the other processes are idle.

  Scaling by the max is a reduction followed by a broadcast.
  Again, solutions based on gather are sub-optimal.

  Printing the max can be done with a gather,
  or do a broadcast of the max and whoever has that value
  prints. \\
  Another option is a reduction on a data~/ index pair;
  if someone comes up with that, mention that it exists,
  otherwise ignore.
\end{instructor}
...
\begin{instructor}
  Talk about `distributed array': a~mental construct of
  a very large array that is spread over the processes.\\
  Each process has a local array numbered zero-to-whatever,
  but conceptually they are a very large array.

  Remember; one of the reasons for MPI is that on multiple machines
  you can handle larger problems.
\end{instructor}
...
\begin{instructor}
  Mathematically inclined students may know about different norms:
  \[ \|\cdots\|_2, \|\cdot\|_1, \|\cdot\|_\infty \]
  No harm if not: they all know about sum-of-squares.
\end{instructor}
...
\begin{instructor}
  Time bottleneck: all the computation happens on the root

  Memory bottleneck: root needs to have as much memory as all
  processes combined.
\end{instructor}
...
the standard deviation is printed out, and not further used
in computations.
...
\begin{numberedframe}{Allreduce arguments}
  Scalar buffer
\begin{lstlisting}
double in_x,out_x;
MPI_Allreduce( &in_x,&out_x,1,MPI_DOUBLE, ... )
\end{lstlisting}
Note: count$=1$ because reduction on single scalar.

General buffer:
\begin{lstlisting}
double in_x[5],out_x[5];
MPI_Allreduce( in_x,out_x,5,MPI_DOUBLE, ... )
\end{lstlisting}

C++
\begin{lstlisting}
vector<double> x(5);
MPI_Comm_allreduce( x.data(), ...,5,... )
\end{lstlisting}

\begin{itemize}
\item \indexmpishow{MPI_Datatype} is
  \indexmpishow{MPI_INT}, \indexmpishow{MPI_FLOAT} (for~C),
  \indexmpishow{MPI_REAL8} (for Fortran) et cetera.
...
\begin{instructor}
  Point out the obvious ones.

  Operators such as \lstinline{MPI_MAXLOC} are tricky to use.
\end{instructor}
...
\begin{instructor}
  The code has suggestively named variables,
  so this exercise `should be obvious'.

  Point out the correctness test and the use of computer arithmetic in it:\\
  single precision is $10^{-7}$ accurate, so count on 5 or~6 digits;
  double is~$10^{-16}$ so 14 or~15 digits.\\
  But it depends on the algorithm!
\end{instructor}
...
\begin{instructor}
  Point out that the sendbuffer is omitted;
  every places their data in the receive buffer.
\end{instructor}
...
\begin{instructor}
  Ask in class: `why is there only one buffer? Why not
...
\begin{instructor}
  Draw an irregular object (airplane?)
  and its division over processes.

  Now every process knows how many elements it has,
  it needs to apply scan to ask what the global number
  of its first element is.
\end{instructor}
...
\begin{instructor}
  Originally there was only inclusive scan:
  if you needed the exclusive, you could `divide out' the local element.

  Class: when does that not work? Answer: operations like max or logical-and.
\end{instructor}
...
\mpiproto{MPI_Gather}
\begin{instructor}
  Point out the two buffer-count-datatype triples.

  Important: the count in the long buffer is still
...
\begin{instructor}
  I know of only two applications: sparse matrix setup
  and the scalable dense matrix-vector product.

  They haven't seen the sparse picture yet (it's in point-to-point)
  and the scalable dense will come later in theory,
  so go easy on this.
\end{instructor}
...
\begin{mpl}
\begin{numberedframe}{User-defined operators, MPL}
  General type:
\begin{lstlisting}[numbers=none]
T(T&)
\end{lstlisting}
