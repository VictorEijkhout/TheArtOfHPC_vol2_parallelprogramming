...
\begin{instructor}
  The datatype variable is of type \lstinline{MPI_Datatype};
  another use of the C~PreProcessor.
  
  Don't forget the commit. It does some internal optimization,
  like `can a recursively defined type be simplified?'

  Note that the type is always passed by reference,
  so the `free' call is a de-allocation and the pointer is nulled.
  Omitting the free call gives memory leaks
\end{instructor}
...
\begin{instructor}
  ...
  You can imagine that your code always assumes a data type,
  sometimes contiguous, sometimes strided.
  In that case using the contiguous type makes the code more elegant.
\end{instructor}
...
\begin{instructor}
  Note that the stride is from start of block to start of block;
  it is not the gap.
\end{instructor}
...
\begin{instructor}
  Computing the displacements is tricky:
  it can involve padding of types.

  Therefore we consider packing. Next.
\end{instructor}
...
\begin{instructor}
  This examples is a `self-documenting message'.\\
  Compare this to the python \lstinline{pickle} module.\\
  The `native' interface to \texttt{mpi4py} probably uses
...
\begin{instructor}
  Talk the class through how a 2D subsection can be done
  through a Vector type.\\
  What are blocksize, count, and stride?

  However, a 3D subsection will probably be a
  vector-of-vectors and very tedious. \\
  Hence the SubArray type.
\end{instructor}
...
\begin{instructor}
  Note that \lstinline{max elements} is used as the size of the receive buffer,
  but fewer elements than that are received.
\end{instructor}
...
\mpiproto{MPI_Type_create_resized}
