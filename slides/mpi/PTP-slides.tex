...
\begin{numberedframe}{MPI point-to-point mechanism}
  \begin{multicols}{2}
    \includegraphics[scale=.12]{passnote}
    \columnbreak
    \begin{itemize}
...
\mpiprotocombined{MPI_Send}
\mpiprotocombined{MPI_Recv}
...
MPI_Status_get_source( &status,&s ); // mpi-4
int t = status.MPI_TAG;
MPI_Status_get_tag( &status,&t ); // mpi-4
MPI_Status_get_error( &status,&e ); // mpi-4
...
\mpiprotocombined{MPI_Wtime}
\mpiprotocombined{MPI_Wtick}
...
\begin{numberedframe}{Halo regions}
  \label{sl:halo-buffer}
  Each process needs buffers for receiving data
  from the left and right:
  
  \includegraphics[scale=.5]{halo1d}

  (This complicates the code from the single-processor case. Agree?\\
...
\begin{exerciseframe}[rightsend]
  ...
  (This is half of the three-point stencil computation;\\
  see slide~\ref{sl:halo-buffer})
\end{exerciseframe}
...
\begin{numberedframe}{Synchronous send}
  ...
  \item No `eager' sends
  \item $\Rightarrow$~enforce always blocking behavior\\
    $\Rightarrow$~reproducible behavior    
  \end{itemize}
\end{numberedframe}

\mpiprotocombined{MPI_Ssend}
...
\end{lstlisting}
\begin{itemize}
\item Status object or \indexmpishow{MPI_STATUS_IGNORE};
\item Request gets deallocated.
\end{itemize}
...
\end{lstlisting}
\begin{itemize}
\item Ignore status array: \indexmpishow{MPI_STATUSES_IGNORE};
\item Other wait calls:
  \indexmpishow{MPI_Wait}, \indexmpishow{MPI_Waitany}, \indexmpishow{MPI_Waitsome}.
\end{itemize}
\end{numberedframe}
...
\end{lstlisting}
\begin{itemize}
\item Can you flip the Isend/Irecv?
\item Can you change the Recv/Send for process~B to Irecv/Isend?
\end{itemize}
\end{numberedframe}
\begin{instructor}
  \begin{itemize}
  \item Process A can switch the two because non-blocking calls
    are unordered.
  \item Changing the process~B calls is not allowed: they need to be in that order.
    You could sequence them as Irecv/Wait/Isend/Wait, but that's pointless.
  \end{itemize}
\end{instructor}
