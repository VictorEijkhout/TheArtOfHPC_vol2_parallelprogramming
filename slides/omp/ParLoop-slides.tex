...
%%%%
%%%% ParLoop-slides.tex : slides about OpenMP's parallel loops
%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{numberedframe}{Parallelizing Loops }
 Put \indexompshow{for} directive before a loop:
\begin{lstlisting}
#pragma omp parallel
{
#pragma omp for
...
\begin{instructor}
  A long time ago there was the dream of a `parallelizing compiler':
  a~compiler that would look at the code and make a parallel version.

  This dream is long dead.
  It's more productive for the programmer to tell the compiler
  that a loop is parallel.
\end{instructor}
...
\begin{exerciseframe}[nestedloops]
  Run the following snippet
  \cverbatimsnippet{matexchseq}
  \begin{enumerate}
  \item Sequentially
  \item On one thread
...
\lstinline{firstprivate}: private, but initialized
\begin{lstlisting}
float x=5.5;
...
\lstinline{lastprivate}: private variable persisting after the loop
\begin{lstlisting}
float y;
