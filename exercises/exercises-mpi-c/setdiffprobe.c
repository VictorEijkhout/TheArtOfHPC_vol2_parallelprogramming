...
int main(int argc,char **argv) {
   ...
   *    by passing the filter to every other process, non-blocking
   */

  // 1. apply the filter locally
  setdiff( result,filter,N );

  // 2. use non-blocking sends to send filter to everyone else
  MPI_Request sendreqs[nprocs];
  for (int target=0; target<nprocs; target++) {
    // Send my filter to everyone else
/**** your code here ****/
  }

  /*
   * 3. receive everyone else's filter
   *   note that there is only one buffer
   *   what does that imply for the kind of receive?
   */
  int incoming[N];
  // Receive all other filters
  for (int stage=0; stage<nprocs-1; stage++) {
/**** your code here ****/
    setdiff( result,incoming,N );
  }
  MPI_Waitall(nprocs,sendreqs,MPI_STATUSES_IGNORE);
