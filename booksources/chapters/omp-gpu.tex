...
Test whether you are on device with the function \indexompshow{omp_is_initial_device}
which return 1~for the host, the initial device, and 0~for the \ac{GPU},
the attached device.

The function \indexompshow{omp_get_num_devices} returns the count
of attached devices.

Here is a simple example using only the \indexompshow{target} clause:
\cverbatimsnippet{ompdetectgpus}
