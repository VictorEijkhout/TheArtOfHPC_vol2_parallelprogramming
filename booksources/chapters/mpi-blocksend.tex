...
\begin{figure}[ht]
  \includegraphics[scale=.7]{halo1d}
  \caption{Halo regions for left/right communication}
  \label{fig:halo1d}
\end{figure}

If we implement this scheme we encounter a phenomenon
known as a~`halo region':
a~process needs some buffer space for storing the data
that comes from the left and/or right.
This is illustrated in figure~\ref{fig:halo1d}.

For now, in the case of only passing data from the left neighbor,
only one buffer is needed, but in more general cases
more buffer space can be required.
The case of two-dimensional \acp{PDE} is discussed
in \HPSCref{sec:spmvp}.
...
\begin{exercise}
  ...
  Revisit exercise~\ref{ex:linear-sequential} and solve it using
