...
with members that can be access in two ways.
\begin{itemize}
\item The function \indexmpishow{MPI_Status_get_source} (\mpistandard{4})
  returns the \indexmpishow{MPI_SOURCE} member that  gives the source of.the message;
  see section~\ref{sec:mpi-source}.
\item The function  \indexmpishow{MPI_Status_get_tag} (\mpistandard{4})
  return the \indexmpishow{MPI_TAG} member that
  gives the tag with which the message was received;
  see section~\ref{sec:mpi-tag}.
\item The function \indexmpishow{MPI_ERROR} (\mpistandard{4})
  returns the  \indexmpishow{MPI_Status_get_error} member
  that gives the error status of the receive call;
  see section~\ref{sec:mpi-status-error}.
\item The function \indexmpishow{MPI_Get_count}
...
For symmetry, there is a function \indexmpishow{MPI_Status_set_source}
to set the source recorded in a status object.
...
For symmetry there is a function \indexmpishow{MPI_Status_set_tag}
to set the tag as recorded in a status object.
...
For symmetry there is a function \indexmpishow{MPI_Status_set_error}.
to set the error as recorded in a status object.
...
In chapter~\ref{proj:mandelbrot} you can do programming project with
this model.
