...
\Level 1 {Callback and Context}

Certain PETSc routines take a
\indexterm{context} argument, for instance
\begin{lstlisting}
#include "petscmat.h" 
...
Some routines have an explicit argument for destroying the context:
\begin{lstlisting}
#include "petscdmshell.h"  

PetscErrorCode DMShellSetContext(DM dm, void *ctx);
PetscErrorCode DMShellSetDestroyContext(DM dm, PetscCtxDestroyFn *f);
\end{lstlisting}

The destroy routines have
\indexpetscdef{PetscCtxDestroyFn} as the prototype for all context destroy functions.
It is
...
for scalars, or
\indexpetsctt{PETSC_NULL_INTEGER_ARRAY}, \indexpetsctt{PETSC_NULL_SCALAR_ARRAY},
 \indexpetsctt{PETSC_NULL_REAL_ARRAY} for array arguments.
...
As of version 3.24,
a variable \indextermtt{PETSCPYTHONPATH} is added to the generated
\lstinline{lib/petsc/conf/}\indexpetscshow{petscvariables} file
so it is available to users through the makefile system.
This variable is also added to \indexpetscshow{PETSc.pc}
so that it can be accessed as
\begin{lstlisting}
pkg-config --variable=PETSCPYTHONPATH PETSc.pc
\end{lstlisting}
