...
\begin{exercise}
  ...
  \item\label{ex:coll-cases-allreduce}
    Each process computes a random number again.
    Each process wants to scale its number by the maximum computed
    in the previous question.
  \item Let each process compute a random number. You want to print on what process the
...
You already saw that in exercise~\ref{ex:collective-cases},
item~\ref{ex:coll-cases-allreduce}.

A practical example would be the computation of the \indexterm{standard deviation}:
