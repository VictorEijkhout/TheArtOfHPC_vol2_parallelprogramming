...
\Level 0 {Application Binary Interface (ABI)}
\label{sec:mpi-abi}

As of \mpistandard{4}, MPI has an \acf{ABI},
which specifies
\begin{itemize}
\item the memor layout of opaque handle objects, and
\item the values of integer constants, as well as
\item other aspects that require a defined \ac{ABI}.
\end{itemize}

The version of the \ac{ABI} is available:
\begin{itemize}
\item through compile-time constants
  \indexmpidef{MPI_ABI_VERSION} and \indexmpidef{MPI_ABI_SUBVERSION}; and
\item through a query function
  \indexmpidef{MPI_Abi_get_version}, returning
  the major and minor version
  (or \lstinline{-1} if not suported).
\end{itemize}

Further information on the \ac{ABI} can be obtained through 
\indexmpidef{MPI_Abi_get_info},
returning a~(pointer to~an) \indexmpishow{MPI_Info} object.
%
The \ac{ABI} info object contains at least the following
predefined keys:
\begin{itemize}
\item \indexmpidef{mpi_aint_size} for the size of
  an \indexmpishow{MPI_Aint} in bytes;
\item \indexmpidef{mpi_count_size} for the size of
  an \indexmpishow{MPI_Count} in bytes;
\item \indexmpidef{mpi_offset_size} for the size of
  an \indexmpishow{MPI_Offset} in bytes;
\end{itemize}

\Level 1 {Header and library}

The (?any?) \ac{ABI} must be implemented with a header \lstinline{mpi.h},
and the library implementing the standard \ac{ABI} must be named \lstinline{mpi_abi},
plus any prefix and/or suffix, for instance \lstinline{libmpi_abi.so}.

\Level 0 {Versioning}
\label{sec:mpi-version}

There are a number of routines for querying versions.

The \emph{MPI standard version}\index{MPI!standard!version} is available:
\begin{itemize}
\item  through compile-time parameters
  \indexmpidef{MPI_VERSION} and \indexmpidef{MPI_SUBVERSION};
\item through the function \indexmpiref{MPI_Get_version}.
...
The \emph{MPI library version}\index{MPI!library!version} is available
only through a function \indexmpiref{MPI_Get_library_version},
which returns a string of length maximally \indexmpiref{MPI_MAX_LIBRARY_VERSION_STRING}.

As of \mpistandard{5}, MPI has a defined \ac{ABI}; see section~\ref{sec:mpi-abi}.
Its version and subversion can be queried with
\indexmpiref{MPI_Abi_get_version} and \indexmpiref{MPI_Abi_get_info}.
...
\item \indexmpishow{MPI_IO}
  rank of a process that has regular I/O facilities.
  Processes in the same communicator may return different values
