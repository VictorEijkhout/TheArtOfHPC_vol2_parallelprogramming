...
the variable \lstinline{count} is declared outside the
parallel region and is therefore shared. When the print statement is
executed, all tasks will have been generated, and so \lstinline{count} will be
zero. Thus, the output will likely be \lstinline{0,50}.

In the second example,
the \lstinline{count} variable is private to the thread creating the tasks,
...
z = h(y);  
\end{lstlisting}
Explanation: when the statement computing \lstinline{z} is executed, the task
computing~\lstinline{y} has only been scheduled;
...
\item Dependencies can be based on scalars or on array sections:
\begin{lstlisting}
#pragma omp task depend(in:array[0:5]) depend(out:barray[6:])
\end{lstlisting}
...
\begin{exercise}
  \label{ex:taskio}
  Assume input files \lstinline[language=shell]{input0,input1,...},
  each containing a single integer.
  The following code reads these integers, transforms them into another integer,
  and then writes out the results to files
  \lstinline[language=shell]{output0,output1,...}:
\begin{lstlisting}
for ( int ifile=0; ifile<nfiles; ifile++ ) {
  inputs[ifile] = read_input(ifile);
  outputs[ifile] = transform( inputs,ifile );
  write_output( ifile,outputs[ifile] );
}
\end{lstlisting}
  With the \lstinline{transform} function defined as:
\begin{lstlisting}
...
\end{lstlisting}
Use a task for each of the three lines in the loop body.
Use dependencies to ensure correct synchronization.
\end{exercise}
