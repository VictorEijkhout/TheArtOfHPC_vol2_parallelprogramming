...
and explicitly indicate a `layout'.

The simplest layout is a \indexmplshow{contiguous_layout}:
\begin{lstlisting}
...
More about layouts later; 
see note~\ref{mpl:derived} and on.
...
\begin{mplnote}{Array buffers}
  \label{mpl:vec-arr}
  You can pass a C-style `automatic array' as buffer, requiring a layout:
...
\begin{mplnote}{Send vs recv buffer}
  ...
  \mplverbatimsnippet{mplreduceroot}%also mplmaxonroot
\end{mplnote}
