...
There are various way to allocate memory.
\begin{itemize}
\item \indexpetscdef{PetscMalloc} is analogous to \lstinline{malloc}:
\begin{lstlisting}
PetscInt *ii;
PetscMalloc( 10*sizeof(PetscInt),&ii );
\end{lstlisting}
\item the routine \indexpetscdef{PetscMalloc1} does the \lstinline{sizeof} for you:
\begin{lstlisting}
PetscInt *ii;
PetscMalloc1( 10,&ii );
\end{lstlisting}
\item to have additionally the memory zeroed, use \indexpetscdef{PetscCalloc1}
\begin{lstlisting}
PetscInt *ii;
PetscCalloc1( 10,&ii );
\end{lstlisting}
...
There is also \indexpetscdef{PetscNew}
and \indexpetscref{PetscFree}.
...
with \indexpetscdef{PetscMallocDump}.
%
The commandline option \indexpetscshow{-malloc_dump} outputs all
not-freed memory during \indexpetscshow{PetscFinalize}.
