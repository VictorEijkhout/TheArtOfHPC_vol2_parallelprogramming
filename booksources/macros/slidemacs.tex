...
\def\mplprotoslideinclude{%
  \begin{frame}[containsverbatim]\frametitle{\texttt{MPL: \protoname}}
    ...
    \edef\tmp{\lowercase{\def\noexpand\standardroutine{\protoname}}}\tmp
    \IfFileExists
        {standardx/\standardroutine.tex}
        {\input{standardx/\standardroutine}}
...
\newenvironment
    ...
    {%% if there is an optional tag, reuse the previous slide number
      \def\empty{}\def\opt{#1}\ifx\empty\opt \refstepcounter{slidecount} \fi
      \begin{frame}[containsverbatim]{\arabic{slidecount}#1.\ #2}
    }
    {\end{frame}}
